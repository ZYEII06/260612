class Game1 {
    ...
    reset() {
        ...
        this.config = (DIFFICULTY_CONFIG && DIFFICULTY_CONFIG[currentDifficulty]) ? DIFFICULTY_CONFIG[currentDifficulty].game1 : {};
        ...
        this.sliders = [
            { id: 'r', label: 'R', val: 127, y: 430, color: [255, 107, 107] },
            { id: 'g', label: 'G', val: 127, y: 480, color: [78, 205, 196] },
            { id: 'b', label: 'B', val: 127, y: 530, color: [51, 154, 240] }
        ];
        this.submitBtn = { x: this.canvasX + CANVAS_WIDTH / 2, y: 590, w: 140, h: 44 };
    }
    ...
    drawGameCanvas() {
        ...
        textSize(13);
        textStyle(BOLD);
        text('目標顏色 (Target Color)', this.canvasX + 25, 102);
        ...
        fill(255);
        stroke(220);
        rect(this.canvasX + 10, 240, CANVAS_WIDTH - 20, 310, 12);

        fill(80);
        textStyle(BOLD);
        text('屬性 (Properties)', this.canvasX + 25, 245);
        textStyle(NORMAL);

        // 玩家目前顏色預覽
        fill(this.sliders[0].val, this.sliders[1].val, this.sliders[2].val);
        stroke(220);
        rect(this.canvasX + 25, 275, CANVAS_WIDTH - 50, 100, 8);
        
        noStroke();
        fill(255);
        text('你的顏色 (Your Color)', this.canvasX + 35, 290);
        ...
        this.sliders.forEach(s => {
            ...
            stroke(255);
            strokeWeight(2);
            circle(hx, s.y, 22); // 稍微放大一點更好拉
            
            // 把手上的裝飾線 (讓它看起來可以抓)
            stroke(255, 150);
            line(hx - 3, s.y - 5, hx - 3, s.y + 5);
            line(hx + 3, s.y - 5, hx + 3, s.y + 5);
        ...
        if (this.showFeedback > 0) {
            ...
            textSize(22);
            textStyle(BOLD);
            text(this.lastSimilarity + "%", this.canvasX + CANVAS_WIDTH / 2, 325);
            textStyle(NORMAL);
        }
    }
